...
data structures and caches to support testing.
...
class Request(object):
    ...
    to do.
    """
    ABBREVIATED_LENGTH = 4
    COMMANDS = dict(a2z=1, address=2, entry=2, qd=1, cache=3, evictions=2, refresh=2, quit=1)
    ABBREVIATED = { k for k in COMMANDS.keys() if len(k) > ABBREVIATED_LENGTH }
    ...
    def validate_request(self, request):
        ...
        if len(verb) >= self.ABBREVIATED_LENGTH:
